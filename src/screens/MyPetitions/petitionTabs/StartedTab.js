...
import { startedPetitionsFetch } from "../../../Redux/Actions/startedSignedPetitions";


const Started = () => {
  const dispatch = useDispatch();
  const startedPosts = useSelector((state) => state.startedSigned.started);

  const history = useHistory();

  useEffect(() => {
    dispatch(startedPetitionsFetch());
  }, [dispatch]);
